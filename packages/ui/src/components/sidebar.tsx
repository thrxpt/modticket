...
import { Skeleton } from "@modticket/ui/components/skeleton";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
} from "@modticket/ui/components/tooltip";
import { useIsMobile } from "@modticket/ui/hooks/use-mobile";
...
function SidebarProvider({
  ...
  const [_open, _setOpen] = React.useState(() => {
    ...
    return cookieValue === null ? defaultOpen : cookieValue;
  });
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      className={cn(
        "relative flex min-w-0 flex-1 flex-col overflow-x-hidden bg-background md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2 md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm",
        className
      )}
